<?php
    class post_model
    {
        ...
        public function get_post_favorite_userID($userID)
        {
            try 
            {
                $this->open_kn();
                $sql = "SELECT * FROM `posts` INNER JOIN `favorite_post` ON favorite_post.postID = posts.postID 
                                WHERE favorite_post.userID = ?";
